...
class AdaptiveBatchNorm2d(nn.Module):
    def __init__(self, pretrained_bn, myalpha=0.05):
        super(AdaptiveBatchNorm2d, self).__init__()
        self.pretrained_bn = pretrained_bn
        self.alpha = myalpha
    ...
    def forward(self, input):
        ...
        running_mean = (1 - self.alpha) * self.pretrained_bn.running_mean + self.alpha * batch_mean
        running_var = (1 - self.alpha) * self.pretrained_bn.running_var + self.alpha * batch_var

        return F.batch_norm(
            input, running_mean, running_var, self.pretrained_bn.weight, self.pretrained_bn.bias,
            training=self.training, momentum=0, eps=self.pretrained_bn.eps)
...
class BN_layer(nn.Module):
    ...
    def forward(self, x):
        ...
        # print('l1:', l1.shape, 'l2:', l2.shape, 'x[2]:', x[2].shape)
